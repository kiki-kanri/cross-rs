...
# Change sources if /etc/apt/sources.list.d/ubuntu.sources is detected.
[ -f /etc/apt/sources.list.d/ubuntu.sources ] && sed -i 's|http://.*archive.ubuntu.com|http://free.nchc.org.tw|g; s|http://.*security.ubuntu.com|http://free.nchc.org.tw|g' /etc/apt/sources.list.d/ubuntu.sources
